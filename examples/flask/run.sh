...
$PY -m lenscheck_contract.cli export --no-django --root before --app app:app -o /tmp/fl_base.json >/dev/null 2>&1
$PY -m lenscheck_contract.cli export --no-django --root after  --app app:app -o /tmp/fl_head.json >/dev/null 2>&1

echo "${cyan}── Layer 2 · ${b}lenscheck-contract diff${r}${cyan}  (structural — runs in CI) ──${r}"
echo
$PY -m lenscheck_contract.cli diff /tmp/fl_base.json /tmp/fl_head.json --markdown --fail-on risky
code=$?
echo
